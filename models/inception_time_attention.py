...
class InceptionModule2D(nn.Module):
    """保留原始的InceptionModule2D以兼容性"""
    def __init__(self, in_channels, bottleneck_channels=32, kernel_sizes=[5, 11, 23]):
        super().__init__()
        self.bottleneck = nn.Conv2d(in_channels, bottleneck_channels, kernel_size=1, bias=False)
        self.convs = nn.ModuleList(
            [nn.Conv2d(bottleneck_channels, bottleneck_channels, kernel_size=(1, k), padding='same') for k in
             kernel_sizes])
        self.maxpool_conv = nn.Sequential(nn.MaxPool2d(kernel_size=(1, 3), stride=1, padding=(0, 1)),
                                          nn.Conv2d(in_channels, bottleneck_channels, kernel_size=1, bias=False))
        total_out_channels = bottleneck_channels * (len(kernel_sizes) + 1)
        self.bn = nn.BatchNorm2d(total_out_channels)
...
class SpectrogramInceptionTimeAttention2D(nn.Module):
...
class InceptionTimeAttention2D(nn.Module):
    """保留原始的InceptionTimeAttention2D以兼容性"""
    def __init__(self, input_shape=(1, 129, 47), num_classes=9, num_modules=2, hidden_channels=32):
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv2d(1, hidden_channels, kernel_size=(3, 3), padding='same', bias=False),
            nn.BatchNorm2d(hidden_channels),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))
        )
        ...
        current_channels = hidden_channels
        for i in range(num_modules):
            module_out_channels = hidden_channels * 4
            self.inception_modules.append(InceptionModule2D(current_channels, bottleneck_channels=hidden_channels))
            if current_channels != module_out_channels:
                self.residual_projections.append(
                    nn.Conv2d(current_channels, module_out_channels, kernel_size=1, bias=False))
            else:
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(), nn.Linear(current_channels, 128), nn.ReLU(),
            nn.Dropout(0.7), nn.Linear(128, num_classes)
        )
